...
//
// Header-only .env loader with three-tier precedence:
//   std::getenv(name)  >  .env file  >  caller default
//
// Mirrors the Python env.py precedence.  The .env file is auto-discovered by
// walking up from the working directory (and falling back to
// /proc/self/exe), then parsed once into a static cache.  Parsing is
// intentionally boring — KEY=VALUE per line, # comments, optional
// surrounding quotes, no interpolation.
//
// This is a production header (no test dependency): Is310P()/Is910B() and
// benchmark's GetModelDir() both resolve ASCEND_PLATFORM / model dir through
// GetEnv() so that a bare `./benchmark` invocation (no .env sourced into the
// shell) still picks up .env the same way ctest does.
//
// Usage:
