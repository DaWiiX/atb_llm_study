 ...
 *   /tmp/bicubic_<name>_pil_output.bin   — PIL.Image.resize(BICUBIC), the
 *                                          bit-exact ground truth for the C++
 *                                          CPU BicubicResize (PIL 8bpc port).
 *                                          (_output.bin is a legacy alias = PIL)
 *
 * Cases:
...
TEST_CASE("PreprocessImage CPU vs official transformers") {
    // CPU-only precision gate: the pure-CPU PreprocessImage (SmartResize +
    // PIL 8bpc bicubic + normalize + patch extraction) vs the *official*
    // Qwen3VL transformers preprocessor at all 4 production resolutions.
    //
    // No CreateRuntime, no Is910B guard — this runs on every platform. The
    // CPU BicubicResize is the PIL 8bpc fixed-point port, so it matches the
    // official PIL-based reference up to the final fp16 quantization. The
    // residual max_diff is purely fp16(CPU) vs torch-fp32(official); cosine
    // stays ≈0.9999.
    //
    // Input-image sharing: this test and the Python generator consume the
    // SAME /tmp/bicubic_prod_<H>x<W>_input.bin (fixed np.random.seed(2026)),
    // so the cosine is meaningful.
    ...
    for (const auto& c : cases) {
        // ── Shared input image (byte-exact with the Python generator) ──
        LoadedArrayF32 in_ref;
        std::string in_path = "/tmp/bicubic_" + std::string(c.name) + "_input.bin";
        if (!in_ref.Load(in_path) || in_ref.shape.size() != 3 ||
            static_cast<int32_t>(in_ref.shape[0]) != channels ||
        ...
        // Cast shared input f32 -> uint8 (clip+round, matching the generator).
        std::vector<uint8_t> image(in_ref.data.size());
        for (size_t i = 0; i < in_ref.data.size(); i++) {
            float v = std::round(in_ref.data[i]);
            v = std::min(255.0f, std::max(0.0f, v));
        ...
        // SmartResize (shared CPU step) — output dims + grid must match official.
        int32_t new_h, new_w;
        atb_llm::adapters::SmartResize(c.h, c.w, factor,
            cfg.pp_min_pixels, cfg.pp_max_pixels, new_h, new_w);
        int64_t grid_h = new_h / cfg.pp_patch_size;
        ...
        // ── CPU full pipeline ──
        std::vector<uint16_t> cpu_out(total_elems);
        int64_t num_patches_cpu = 0;
        int64_t grid_thw_cpu[3] = {0, 0, 0};
        auto s = atb_llm::adapters::PreprocessImage(
            image.data(), channels, c.h, c.w, cfg,
            cpu_out.data(), num_patches_cpu, grid_thw_cpu);
        REQUIRE(s == atb_llm::STATUS_OK);

        CHECK(grid_thw_cpu[0] == 1);
        CHECK(grid_thw_cpu[1] == grid_h);
        CHECK(grid_thw_cpu[2] == grid_w);
        CHECK(num_patches_cpu == num_patches_exp);

        // Convert CPU fp16 -> f32 and cosine vs official fp32.
        std::vector<float> cpu_f32(total_elems);
        for (int64_t i = 0; i < total_elems; i++) {
            cpu_f32[i] = atb_llm::Fp16ToF32(cpu_out[i]);
        }
        double cos = CosineSim(cpu_f32.data(), official.data(), total_elems);
        float  max_d = MaxAbsDiff(cpu_f32.data(), official.data(), total_elems);
        ...
        LOG_INFO("  [%s] %dx%d -> %dx%d grid=(1,%ld,%ld) patches=%ld "
                 ...
                 static_cast<long>(grid_h), static_cast<long>(grid_w),
                 static_cast<long>(num_patches_cpu), cos, max_d,
                 downsample ? "  [PIL 8bpc AA downsample]" : "  [identity]");

        CHECK(cos >= 0.99);
    }

    LOG_INFO("================================================================");
    LOG_INFO("[CPU OFFICIAL GATE] PreprocessImage (CPU) vs official transformers: cos >= 0.99");
    LOG_INFO("  CPU BicubicResize = PIL 8bpc port; residual = fp16 vs torch-fp32");
    LOG_INFO("================================================================");
}
