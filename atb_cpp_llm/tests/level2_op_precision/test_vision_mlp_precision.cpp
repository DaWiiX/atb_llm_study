...
// ═════════════════════════════════════════════════════════════════
// Case: VisionMLP typical Qwen3VL scale
// ═════════════════════════════════════════════════════════════════
TEST_CASE("VisionMlpGraph precision: N=256 H=1024 I=4096 (Qwen3VL)") {
    LOG_INFO("=== VisionMLP precision (Qwen3VL scale) ===");

    ArrayFp16 x, fc1_w, fc1_b, fc2_w, fc2_b, ref;
    MetaI32 meta;
    REQUIRE(x.Load("/tmp/cpu_vision_mlp_typical_x.bin"));
    REQUIRE(fc1_w.Load("/tmp/cpu_vision_mlp_typical_fc1_w.bin"));
    REQUIRE(fc1_b.Load("/tmp/cpu_vision_mlp_typical_fc1_b.bin"));
    REQUIRE(fc2_w.Load("/tmp/cpu_vision_mlp_typical_fc2_w.bin"));
    REQUIRE(fc2_b.Load("/tmp/cpu_vision_mlp_typical_fc2_b.bin"));
    REQUIRE(ref.Load("/tmp/cpu_vision_mlp_typical_ref.bin"));
    REQUIRE(meta.Load("/tmp/cpu_vision_mlp_typical_meta.bin"));
    REQUIRE(meta.data.size() == 3);
    int64_t N = meta.data[0], H = meta.data[1], I = meta.data[2];
    ...
    atb_llm::OperationHandle op;
    REQUIRE(IS_OK(atb_llm::components::VisionMlpGraph::Build("VMLPTypical", op)));
    REQUIRE(op.get() != nullptr);
...
// ═════════════════════════════════════════════════════════════════
// Case: VisionMLP small scale with zero bias (functionally no-bias)
// ═════════════════════════════════════════════════════════════════
TEST_CASE("VisionMlpGraph precision: N=4 H=32 I=64 zero_bias") {
    LOG_INFO("=== VisionMLP precision (small, zero bias) ===");

    ArrayFp16 x, fc1_w, fc1_b, fc2_w, fc2_b, ref;
    MetaI32 meta;
    REQUIRE(x.Load("/tmp/cpu_vision_mlp_nobias_x.bin"));
    REQUIRE(fc1_w.Load("/tmp/cpu_vision_mlp_nobias_fc1_w.bin"));
    REQUIRE(fc1_b.Load("/tmp/cpu_vision_mlp_nobias_fc1_b.bin"));
    REQUIRE(fc2_w.Load("/tmp/cpu_vision_mlp_nobias_fc2_w.bin"));
    REQUIRE(fc2_b.Load("/tmp/cpu_vision_mlp_nobias_fc2_b.bin"));
    REQUIRE(ref.Load("/tmp/cpu_vision_mlp_nobias_ref.bin"));
    REQUIRE(meta.Load("/tmp/cpu_vision_mlp_nobias_meta.bin"));
    REQUIRE(meta.data.size() == 3);
    int64_t N = meta.data[0], H = meta.data[1], I = meta.data[2];
    ...
    atb_llm::OperationHandle op;
    REQUIRE(IS_OK(atb_llm::components::VisionMlpGraph::Build("VMLPNoBias", op)));
    REQUIRE(op.get() != nullptr);
