...
脚本里有一个固定的 `KNOWN_LEVELS` 数组（5 个 level 名）。位置参数命中数组 → 当 level（`ctest -L`）；否则当测试名（`ctest -R`）。**结论**：永远不要把测试取成跟 level 同名，否则会被脚本当过滤器误判。
...
### 8. 跟 `--no-refdata` / `--refresh-refdata` 组合

`--test-only` 默认仍然会调 `gen_all.py --skip-fresh`（只 stat 哨兵文件，亚秒级，不重新生成）。如果你确认参考数据已就绪、连这步都想省，加 `--no-refdata`；如果改了 Python 参考生成脚本要强刷，加 `--refresh-refdata`。
...
| 只跑 Level 1 | `bash build_and_test.sh --test-only level1_cpu_pure` |
| 只跑 1 个测试 | `bash build_and_test.sh --test-only test_vision_stages` |
| 只跑 1 个测试 + 看完整输出 | `bash build_and_test.sh --test-only -v test_vision_stages` |
